: &signer, land_id: u64, location: vector<u8>, size: u64) {
        let land = Land {
            land_id,
            location,
            size,
    ...
    public fun transfer_land_ownership(owner: &signer, new_owner: address, land_id: u64) acquires Land {
        let current_owner = signer::address_of(owner);

        // Access the land by the current owner's address and land_id
        ...
        // Optionally, you can handle payment for the transfer (e.g., fee collection, etc.)
    }

    /// Function to verify the current owner of a land by its land_id.
    public fun verify_ownership(land_id: u64, owner: address): address acquires Land {
        // Borrow the global resource by the land's owner's address
...
module LandTitle::Management {
    ...
    public fun register_land(owner
